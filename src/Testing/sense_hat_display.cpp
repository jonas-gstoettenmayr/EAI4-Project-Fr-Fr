...
void SenseHatDisplay::WritePattern(const std::uint16_t pattern[8][8]) {
  ...
  for (int y = 0; y < 8; ++y) {
    for (int x = 0; x < 8; ++x) {
      senseSetRGB565pixel(x,y,pattern[x][y]);
...
bool SenseHatDisplay::ShowRPS(RPS rps, float confidence)
{
  ...
  switch (rps)
  { // added clearing of the matrix after a certain time
  case rock: // display Rock
    WritePattern(r);
    sleep_for(cShowGestureTime);
    Clear();
    break;

  case paper: // display Paper
    WritePattern(p);
    sleep_for(cShowResultTime);
    Clear();

  case scissors: // display Scissors
    WritePattern(s);
    sleep_for(cShowResultTime);
    Clear();

  case reset: // display gay flag & Resets
    WritePattern(gayflag);
    sleep_for(cShowResultTime);
    Clear();

  default: // display the x when there is none of the rps signs?
    ShowErrorMarker();
    sleep_for(500ms);
    return false;
    break;
